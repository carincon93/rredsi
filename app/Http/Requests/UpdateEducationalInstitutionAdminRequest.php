...
class UpdateEducationalInstitutionAdminRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        return [
            ...
            'email'             => 'required|string|max:191|email|unique:users,email,'.$this->route('educational_institution_admin')->id.',id',
            'document_type'     => 'required|string| max:2',
            'document_number'   => 'required|numeric|min:0|max:9999999999|unique:users,document_number,'.$this->route('educational_institution_admin')->id.',id',
    ...
    protected function prepareForValidation()
    {
        ...
        if($this->node_id != null) {
            $this->merge([
                'node_id' => filter_var($this->node_id, FILTER_SANITIZE_NUMBER_INT),
            ]);
        }
    }
}
