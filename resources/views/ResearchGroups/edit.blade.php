<title>{{"Editar la información del grupo de investigación $researchGroup->name  - ".config('app.name') }}</title>
<x-app-layout>
    <x-slot name="header">
        <div class="col-start-2 col-span-4 md:col-start-1 md:col-span-3 xl:col-start-1 xl:col-span-3">
            <h2 class="font-display text-white text-center md:text-left text-2xl leading-9 font-semibold sm:text-3xl sm:leading-9">
                ...
                <span class="text-base sm:text-lg block text-purple-300">
                    <a class="text-white font-weight underline" href="{{ route('nodes.educational-institutions.faculties.research-groups.index', [$node, $educationalInstitution, $faculty]) }}">Lista de grupos de investigación</a> / Editar grupo de investigación
                </span>
            </h2>
        </div>
                ...
                <form method="POST" action="{{ route('nodes.educational-institutions.faculties.research-groups.update', [$node, $educationalInstitution, $faculty, $researchGroup]) }}">
                    ...
                    <div class="mt-1/6">
                        ...
                        <select id="minciencias_category" name="minciencias_category" class="form-select w-full" required >
                            ...
                            <option {{ old('minciencias_category') == "A1" || $researchGroup->minciencias_category  == "A1" ? "selected" : ""  }} value="A1">A1</option>
                            <option {{ old('minciencias_category') == "B" || $researchGroup->minciencias_category  == "B" ? "selected" : "" }} value="B">B</option>
                            <option {{ old('minciencias_category') == "C" || $researchGroup->minciencias_category  == "C" ? "selected" : "" }} value="B">C</option>
                            <option {{ old('minciencias_category') == "Reconocido" || $researchGroup->minciencias_category  == "Reconocido" ? "selected" : "" }} value="Reconocido">Reconocido</option>
                            <option {{ old('minciencias_category') == "No reconocido" || $researchGroup->minciencias_category  == "No reconocido" ? "selected" : "" }} value="No reconocido">No reconocido</option>
                        </select>
