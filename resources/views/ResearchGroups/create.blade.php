    ...
    <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
        <div class="md:grid md:grid-cols-3 md:gap-4">
            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="POST" action="{{ route('nodes.educational-institutions.research-groups.store', [$node, $educationalInstitution]) }}">
                    @csrf
                    ...
                    <div class="mt-4">
                        ...
                        <x-jet-input id="leader" class="block mt-1 w-full" type="text" min="" max="" name="leader" value="{{ old('leader') }}" required />
                        <x-jet-input-error for="leader" class="mt-2" />
                    </div>

                    <div class="mt-4">
                        <x-jet-label for="gruplac" value="{{ __('Gruplac') }}" />
                        <x-jet-input id="gruplac" class="block mt-1 w-full" type="url" min="" max="" name="gruplac" value="{{ old('gruplac') }}" required />
                        <x-jet-input-error for="gruplac" class="mt-2" />
                    </div>

                    <div class="mt-4">
                        <x-jet-label for="minciencias_code" value="{{ __('Minciencias code') }}" />
                        <x-jet-input id="minciencias_code" class="block mt-1 w-full" type="text" min="" max="" name="minciencias_code" value="{{ old('minciencias_code') }}" required />
                        <x-jet-input-error for="minciencias_code" class="mt-2" />
                    </div>

                    <div class="mt-4">
                        <x-jet-label for="minciencias_category" value="{{ __('Minciencias category') }}" />
                        <select id="minciencias_category" name="minciencias_category" class="block mt-1 p-4 w-full" value="{{ old('minciencias_category') }}" required >
