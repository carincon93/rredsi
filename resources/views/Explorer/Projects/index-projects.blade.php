<title>{{ 'Lista de proyectos - ' . config('app.name') }}</title>
<x-guest-layout>
                ...
                <span class="block text-blue-900 xl:inline">
                    <svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" viewBox="0 0 20 20"
                        ...
                        <path regular"
                            ...
                            transform="translate(0 -8)" fill="#233876" />
                    </svg>
                            ...
                            <div class="p-10 md:mb-0 mb-6 flex flex-col">
                                <div class="rounded bg-gray-50 p-4 transform translate-x-6 -translate-y-6 shadow">
                                    <div class="flex items-center">
                                        <div
                                            class="w-10 h-10 inline-flex items-center justify-center rounded-full bg-indigo-200 text-blue-800 mb-5 flex-shrink-0 p-2">
                                        ...
                                        <div style="padding-top: 0.2em; padding-bottom: 0.2rem"
                                            class="ml-2 inline-flex items-center space-x-1 text-xs px-2 bg-gray-200 text-gray-800 rounded-full mb-4">
                                            <div style="width: 0.4rem; height: 0.4rem" class="bg-gray-50 rounded-full">
                                            </div>
                                            <a href="{{ route('nodes.explorer.searchProjects', [$node, 'search' => $project->projectType->type]) }}"
                                                class="text-gray-400 uppercase ml-2"><small>{{ optional($project->projectType)->type }}</small></a>
                                        </div>
                                    </div>
                                    <div class="flex-grow ">
                                        <a href="{{ route('nodes.explorer.searchProjects.showProject', [$node, $project]) }}"
                                            class="text-center">
                                            <h2 class=" text-xl title-font font-medium mb-3">{{ $project->title }}
                                            </h2>
                                ...
                                @if ($researchTeam)
                                    <div class="rounded bg-white p-4 transform translate-x-6 -translate-y-6 shadow">
                                        <p class="text-gray-400"><small>Institución educativa:
                                                {{ optional($researchTeam->researchGroup)->educationalInstitutionFaculty->educationalInstitution->name }}</small>
                                        </p>
                                        <p class="text-gray-400"><small>Grupo de investigación:
                                                {{ optional($researchTeam->researchGroup)->name }}</small></p>
                                        <p class="text-gray-400"><small>Semillero de investigación:
                                                {{ $researchTeam->name }}</small></p>
                                    </div>
                                @endif
                            </div>
                        @empty
    ...
    <div class="py-2 border-t">
        <div class="p-6 mt-4">
            @forelse ($allKeywords->chunk(5) as $chunk)
                <div
                    class="flex justify-around mt-4 sm:items-center sm:justify-around text-center text-sm text-gray-500 sm:text-left">
                    @forelse ($chunk as $keyword)
                        <a href="{{ route('nodes.explorer.searchProjects', [$node, 'search' => $keyword]) }}"
                            class="ml-1 underline">{{ $keyword }}</a>
                    @empty
