<title>{{'Mis experienceos y servicios'}}</title>
<x-app-layout>
    <x-slot name="header">
        <div class="col-start-2 col-span-4 md:col-start-1 md:col-span-3 xl:col-start-1 xl:col-span-3">
            <h2 class="font-display text-white text-center md:text-left text-2xl leading-9 font-semibold sm:text-3xl sm:leading-9">
                {{ __('Mis experienceos y servicios') }}
                <span class="text-base sm:text-2xl block text-purple-300">
                Mostrar experienceos y servicios
                </span>
            </h2>
        </div>
          ...
          <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
            <div class="grid grid-cols-3 gap-6">
              ...
              <div class="flex-auto w-full mb-1 text-xs space-y-2">
                <label for="Title" class="sm:not-sr-only block text-sm font-medium text-gray-700">
                  Title
                </label>
                <div class=" h-8 mt-1 flex rounded-md shadow-sm">
                  <input readonly="readonly" class=" cursor-default  appearance-none block w-full bg-grey-lighter text-grey-darker border border-grey-lighter rounded-lg  py-4 px-4" placeholder="Title del experienceo" name="Title" value="{{ isset($experience->Title)?$experience->Title:old('Title') }}" id="Title">
                </div>
              </div>
            </div>
            </div>
                        ...
                        {{method_field('DELETE')}}
                            ...
                            <div class=" w-auto text-center text-base sm:w-auto items-center justify-center text-blue-400 group-hover:text-blue-300 font-medium leading-none bg-white rounded-lg shadow-sm group-hover:shadow-lg py-3 px-5 border border-transparent transform group-hover:-translate-y-0.5 transition-all duration-150">   
                                <div class="flex justify-center content-center">
                                    ...
                                    <input type="submit" onclick="return confirm('¿Desea eliminar este experienceo o servicio?')" value="Borrar" >
                                </div>    
                            </div>
                           </a>                            
                    </form>
