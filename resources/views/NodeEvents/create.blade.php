        ...
        <div class="col-start-2 col-span-4 md:col-start-1 md:col-span-3 xl:col-start-1 xl:col-span-3">
            <h2 class="font-display text-white text-center md:text-left text-2xl leading-9 font-semibold sm:text-3xl sm:leading-9">
                ...
                <span class="text-base sm:text-lg block text-purple-300">
                    <a class="text-white font-weight underline" href="{{ route('nodes.events.index', [$node]) }}">Lista de eventos de nodo</a> / Crear evento de nodo
                </span>
            </h2>
        </div>
                ...
                <form method="POST" action="{{ route('nodes.events.store', [$node]) }}">
                    ...
                    <div class="block mt-4">
                        <label for="is_annual_event_yes" class="flex items-center">
                            <input id="is_annual_event_yes" value="1" type="radio" class="form-radio" name="is_annual_event" {{ old('is_annual_event') == 1 ? "checked" : "" }}>
                            <span class="ml-2 text-sm text-gray-600">{{ __('Yes') }}</span>
                        </label>
                        <label for="is_annual_event_no" class="flex items-center">
                            <input id="is_annual_event_no" value="0" type="radio" class="form-radio" name="is_annual_event" {{ old('is_annual_event') != null && old('is_annual_event')  == 0 ? "checked" : "" }}>
