<title>{{"Editar la información del semillero de investigación $researchTeam->name - ".config('app.name') }}</title>
<x-app-layout>
    <x-slot name="header">
        <div class="col-start-2 col-span-4 md:col-start-1 md:col-span-3 xl:col-start-1 xl:col-span-3">
            <h2 class="font-display text-white text-center md:text-left text-2xl leading-9 font-semibold sm:text-3xl sm:leading-9">
                ...
                <span class="text-base sm:text-lg block text-purple-300">
                    <a href="{{ route('nodes.educational-institutions.faculties.research-groups.research-teams.index', [$node, $educationalInstitution, $faculty, $researchGroup]) }}" class="text-white font-weight underline">Lista de semilleros de investigación</a> / Editar semillero de investigación
                </span>
            </h2>
        </div>
            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="POST" action="{{ route('nodes.educational-institutions.faculties.research-groups.research-teams.update', [$node, $educationalInstitution, $faculty, $researchGroup, $researchTeam]) }}">
                    ...
                    <div class="mt-1/6">
                        ...
                        <textarea rows="20" id="overall_objective" name="overall_objective" class="form-textarea border-0 w-full" required >{{ old('overall_objective') ?? $researchTeam->overall_objective }}</textarea>
                        <x-jet-input-error for="overall_objective" class="mt-2" />
                    </div>

                    <div class="mt-1/6">
                        <x-jet-label class="mb-4" for="mission" value="{{ __('Mission') }}" />
                        <textarea rows="20" id="mission" name="mission" class="form-textarea border-0 w-full" required >{{ old('mission') ?? $researchTeam->mission }}</textarea>
                        <x-jet-input-error for="mission" class="mt-2" />
                    </div>

                    <div class="mt-1/6">
                        <x-jet-label class="mb-4" for="vision" value="{{ __('Vision') }}" />
                        <textarea rows="20" id="vision" name="vision" class="form-textarea border-0 w-full" required >{{ old('vision') ?? $researchTeam->vision  }}</textarea>
                        <x-jet-input-error for="vision" class="mt-2" />
                    </div>

                    <div class="mt-1/6">
                        <x-jet-label class="mb-4" for="regional_projection" value="{{ __('Regional projection') }}" />
                        <textarea rows="20" id="regional_projection" name="regional_projection" class="form-textarea border-0 w-full" required >{{ old('projection') ??  $researchTeam->regional_projection }}</textarea>
                    ...
                    <div class="mt-1/6">
                        ...
                        <textarea rows="20" id="knowledge_production_strategy" name="knowledge_production_strategy" class="form-textarea border-0 w-full" required >{{ old('knowledge_production_strategy') ?? $researchTeam->knowledge_production_strategy  }}</textarea>
                    ...
                    <div class="mt-1/6">
                        ...
                        <textarea rows="20" id="thematic_research" name="thematic_research" class="form-textarea border-0 w-full" required >@if(old('thematic_research')){{ old('thematic_research')}}@else @foreach(json_decode($researchTeam->thematic_research) as $thematic_research){{ $thematic_research }}@endforeach @endif</textarea>
                        <x-jet-input-error for="thematic_research" class="mt-2" />
                    </div>

                    <p class="mt-1/6">{{ __('Research lines') }} asociadoos</p>
