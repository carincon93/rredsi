...
@props(['prueba'=> null])
    <div class="w-1/2 md:w-1/3 lg:w-64 fixed md:top-0 md:left-0 h-screen lg:block bg-gray-100 border-r z-30" :class="sideBarOpen ? '' : 'hidden'" id="main-nav">
                ...
                <div class="animate-pulse h-16 border-b px-4 items-center mb-8 flex">
                    <a class="pl-16" href="{{ route('nodes.dashboard', [request()->route('node') ? request()->route('node')->id : 1]) }}"><!-- Quemado -->
                ...
                <p class="pl-4 text-sm font-semibold mb-1">{{ __('Node') }} {{$prueba}}</p>
                    ...
                    <div class="w-full flex items-center text-blue-400 h-10 pl-4 hover:bg-gray-200 rounded-lg cursor-pointer">
                      ...
                      <span class="text-gray-700">{{ _('Node events') }}</span>
                    </div>
                    <div class="w-full flex items-center text-blue-400 h-10 pl-4 hover:bg-gray-200 rounded-lg cursor-pointer">
                      <svg class="h-6 w-6 fill-current mr-2" viewBox="0 0 20 20">
                      ...
                      <span class="text-gray-700">{{ _('Educational institutions') }}</span>
                    </div>
                    <div class="w-full flex items-center text-blue-400 h-10 pl-4 hover:bg-gray-200 rounded-lg cursor-pointer">
                      <svg class="h-6 w-6 fill-current mr-2" viewBox="0 0 20 20">
                  ...
                  @if( request()->route('faculty') )
                    ...
                    <div class="mb-4 px-4" id="moduls">
                        <p class="pl-4 text-sm font-semibold mb-1">{{ _('Modulos') }}</p>

                        <a id="academic_programs" class="w-full flex items-center text-blue-400 h-10 pl-4 hover:bg-gray-200 rounded-lg cursor-pointer">
                          <svg class="h-6 w-6 fill-current mr-2" viewBox="0 0 20 20">
                        ...
                        <a  id="research_groups" class="w-full flex items-center text-blue-400 h-10 pl-4 hover:bg-gray-200 rounded-lg cursor-pointer">
                            <svg class="h-6 w-6 fill-current mr-2" viewBox="0 0 20 20">
                            <path d="M14.613,10c0,0.23-0.188,0.419-0.419,0.419H10.42v3.774c0,0.23-0.189,0.42-0.42,0.42s-0.419-0.189-0.419-0.42v-3.774H5.806c-0.23,0-0.419-0.189-0.419-0.419s0.189-0.419,0.419-0.419h3.775V5.806c0-0.23,0.189-0.419,0.419-0.419s0.42,0.189,0.42,0.419v3.775h3.774C14.425,9.581,14.613,9.77,14.613,10 M17.969,10c0,4.401-3.567,7.969-7.969,7.969c-4.402,0-7.969-3.567-7.969-7.969c0-4.402,3.567-7.969,7.969-7.969C14.401,2.031,17.969,5.598,17.969,10 M17.13,10c0-3.932-3.198-7.13-7.13-7.13S2.87,6.068,2.87,10c0,3.933,3.198,7.13,7.13,7.13S17.13,13.933,17.13,10"></path>
                            </svg>
                          ...
                          <a id="educational_environments" class="w-full flex items-center text-blue-400 h-10 pl-4 hover:bg-gray-200 rounded-lg cursor-pointer">
                            <svg class="h-6 w-6 fill-current mr-2" viewBox="0 0 20 20">
                            <path d="M14.613,10c0,0.23-0.188,0.419-0.419,0.419H10.42v3.774c0,0.23-0.189,0.42-0.42,0.42s-0.419-0.189-0.419-0.42v-3.774H5.806c-0.23,0-0.419-0.189-0.419-0.419s0.189-0.419,0.419-0.419h3.775V5.806c0-0.23,0.189-0.419,0.419-0.419s0.42,0.189,0.42,0.419v3.775h3.774C14.425,9.581,14.613,9.77,14.613,10 M17.969,10c0,4.401-3.567,7.969-7.969,7.969c-4.402,0-7.969-3.567-7.969-7.969c0-4.402,3.567-7.969,7.969-7.969C14.401,2.031,17.969,5.598,17.969,10 M17.13,10c0-3.932-3.198-7.13-7.13-7.13S2.87,6.068,2.87,10c0,3.933,3.198,7.13,7.13,7.13S17.13,13.933,17.13,10"></path>
                            </svg>
                          ...
                          <a id="users" class="w-full flex items-center text-blue-400 h-10 pl-4 hover:bg-gray-200 rounded-lg cursor-pointer">
                            <svg class="h-6 w-6 fill-current mr-2" viewBox="0 0 20 20">
                ...
                @if( request()->route('faculty') != null )
                        <input id="val" hidden value="true"></input>
                @else
                        <input id="val" hidden value="false"></input>
                @endif
                ...
                @push('scripts')
                    <script>
                        ...
                        const nodeRoute                         = {{ request()->route('node') != null ? request()->route('node')->id : 0 }};
                        const educationalInstitutionRoute       = {{ request()->route('educational_institution') != null ? request()->route('educational_institution')->id : 0 }};
                        const facultiesRoute                     = {{ request()->route('faculty') != null ? request()->route('faculty')->id : 0 }};

                        @if( request()->route('faculty') )

                            const academic_programs = document.getElementById('academic_programs');
                            const research_groups = document.getElementById('research_groups');
                            const educational_environments = document.getElementById('educational_environments');
                            const users = document.getElementById('users');

                            let uriAcademicPrograms = {!! sprintf('"%s"', route('nodes.educational-institutions.faculties.academic-programs.index', [request()->route('node') ,request()->route('educational_institution'),request()->route('faculty') ]) ) !!};
                            let uriResearchGroups = {!! sprintf('"%s"', route('nodes.educational-institutions.faculties.research-groups.index', [request()->route('node') ,request()->route('educational_institution'),request()->route('faculty') ]) ) !!};
                            let uriEducationalEnvironments = {!! sprintf('"%s"', route('nodes.educational-institutions.faculties.educational-environments.index', [request()->route('node') ,request()->route('educational_institution'),request()->route('faculty') ]) ) !!};
                            let uriUsers = {!! sprintf('"%s"', route('nodes.educational-institutions.faculties.users.index', [request()->route('node') ,request()->route('educational_institution'),request()->route('faculty') ]) ) !!};

                            academic_programs.setAttribute('href', uriAcademicPrograms);
                            research_groups.setAttribute('href', uriResearchGroups);
                            educational_environments.setAttribute('href', uriEducationalEnvironments);
                            users.setAttribute('href', uriUsers);
