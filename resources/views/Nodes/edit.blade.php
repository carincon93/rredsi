        ...
        <div class="md:grid md:grid-cols-3 md:gap-4">
            <div class="md:col-span-1">
                <x-jet-section-title>
                    ...
                    <x-slot name="description">Editar información</x-slot>
                </x-jet-section-title>
            </div>
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="POST" action="{{ route('nodes.update',$node->id) }}">
                    ...
                    <div class="mt-2">
                        ...
                        <select id="administrator_id" name="administrator_id" class="block mt-1 p-4 w-full" value="{{ $node->administrator_id }}" required >
                            <option value="">Seleccione un administrado</option>
                            {{-- @forelse ($nodeAdmins as $nodeAdmin)
                                <option selected="{{ $nodeAdmin->user->id ==  $node->administrator_id  ? 'selected' : '' }}" value={{$nodeAdmin->user->id}}>  {{$nodeAdmin->user->name}} </option>
                            @empty
