                    ...
                    <x-slot name="tbodyData">
                        @foreach ($nodes as $node)
                            ...
                            <tr class="bg-white border-4 border-gray-200">
                                ...
                                <td>
                                    <span class="text-center ml-2 font-semibold">{{$node->administrator_id}}</span>
                                </td>
                                ...
                                <td class="py-2 text-left">
                                    <div class="hidden sm:flex sm:items-center justify-around">
                                        <x-jet-dropdown align="right" width="48">
                                            ...
                                            <x-slot name="content">                        
                                                <x-jet-dropdown-link href="{{ route('nodes.show', $node->id) }}">
                                                    {{ __('Details') }}
                                                </x-jet-dropdown-link>
                                                <x-jet-dropdown-link href="{{ route('nodes.edit', $node->id) }}">
                                                    {{ __('Edit') }}
                                                </x-jet-dropdown-link>
                                                <x-jet-dropdown-link href="{{ route('nodes.destroy', $node->id) }}">
                                                    {{ __('Delete') }}
                                                </x-jet-dropdown-link>
                                            </x-slot>
                                        </x-jet-dropdown>
