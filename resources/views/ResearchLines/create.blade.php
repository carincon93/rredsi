<title>{{'Crear línea de investigación'}}</title>
<x-app-layout>
    <x-slot name="header">
        <div class="col-start-2 col-span-4 md:col-start-1 md:col-span-3 xl:col-start-1 xl:col-span-3">
            <h2 class="font-display text-white text-center md:text-left text-2xl leading-9 font-semibold sm:text-3xl sm:leading-9">
                ...
                <span class="text-base sm:text-lg block text-purple-300">
                    <a href="{{ route('nodes.educational-institutions.faculties.research-groups.research-lines.index', [$node, $educationalInstitution, $faculty, $researchGroup]) }}" class="text-white font-weight underline">Lista de las líneas de investigación</a> / Crear línea de investigación
                </span>
            </h2>
        </div>
        ...
        <div class="md:grid md:grid-cols-3 md:gap-4">
            <div class="md:col-span-1">
                <x-jet-section-title>
                    ...
                    <x-slot name="description">Crear una línea de investigación</x-slot>
                </x-jet-section-title>
            </div>
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="POST" action="{{ route('nodes.educational-institutions.faculties.research-groups.research-lines.store', [$node, $educationalInstitution, $faculty, $researchGroup]) }}">
                    ...
                    <div class="mt-1/6">
                        ...
                        <textarea rows="20" id="objectives" name="objectives" class="form-textarea border-0 w-full" value="{{ old('objectives') }}" required >{{ old('objectives') }}</textarea>
                        <x-jet-input-error for="objectives" class="mt-2" />
                    </div>

                    <div class="mt-1/6">
                        <x-jet-label class="mb-4" for="mission" value="{{ __('Mission') }}" />
                        <textarea rows="20" id="mission" name="mission" class="form-textarea border-0 w-full" value="{{ old('mission') }}" required >{{ old('mission') }}</textarea>
                        <x-jet-input-error for="mission" class="mt-2" />
                    </div>

                    <div class="mt-1/6">
                        <x-jet-label class="mb-4" for="vision" value="{{ __('Vision') }}" />
                        <textarea rows="20" id="vision" name="vision" class="form-textarea border-0 w-full" value="{{ old('vision') }}" required >{{ old('vision') }}</textarea>
                        <x-jet-input-error for="vision" class="mt-2" />
                    </div>

                    <div class="mt-1/6">
                        <x-jet-label class="mb-4" for="achievements" value="{{ __('Achievements') }}" />
                        <textarea rows="20" id="achievements" name="achievements" class="form-textarea border-0 w-full" value="{{ old('achievements') }}" required >{{ old('achievements') }}</textarea>
                        <x-jet-input-error for="achievements" class="mt-2" />
                    </div>

                    <div class="mt-1/6">
                        <x-jet-label class="mb-4" for="knowledge_area_id" value="{{ __('Knowledge area') }}" />
                        <select id="knowledge_area_id" name="knowledge_area_id" class="text-sm mr-10 focus:outline-none rounded-md border-0 p-3.5 shadow-sm block w-full" value="{{ old('knowledge_area_id') }}" required >
