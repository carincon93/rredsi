<title>{{ "Editar información del programa $academicProgram->name - ".config('app.name') }}</title>
<x-app-layout>
    <x-slot name="header">
                ...
                <form method="POST" action="{{ route('nodes.educational-institutions.faculties.academic-programs.update', [$node, $educationalInstitution, $faculty, $academicProgram]) }}">
                    ...
                    <div class="mt-1/6">
                        ...
                        <select id="academic_level" name="academic_level" class="form-select w-full" required >
                            ...
                            <option {{ old('academic_level') == "Tecnólogo" || $academicProgram->academic_level == "Tecnólogo" ? "selected" : "" }} value="Tecnólogo">Tecnólogo</option>
