<title>{{ "Usuarios de la institución educativa - ".config('app.name') }}</title>
<x-app-layout>
    <x-slot name="header">
        <div class="col-start-2 col-span-4 md:col-start-1 md:col-span-3 xl:col-start-1 xl:col-span-3">
            <h2 class="font-display text-white text-center md:text-left text-2xl leading-9 font-semibold sm:text-3xl sm:leading-9">
                ...
                <span class="text-base sm:text-lg block text-purple-300">
                    Lista de usuarios de institución educativa
                </span>
            </h2>
        </div>
        @can('create_educational_institution_user')
                                ...
                                <div class="lg:flex items-center lg:justify-around">
                                    ...
                                    <div class="lg:hidden">
                                        <span class="lg:hidden top-0 left-0 ml-2  px-2 text-gray-400 py-1 text-xs font-bold uppercase block">{{ __('Actions') }}</span>
                                            @can('show_educational_institution_user')
                                                <x-jet-dropdown-link class="inline-block"  href="{{ route('nodes.educational-institutions.faculties.users.show', [$node, $educationalInstitution, $faculty, $user])}}">
                                            ...
                                            @endcan
                                            @can('edit_educational_institution_user')
                                                <x-jet-dropdown-link class="inline-block" href="{{ route('nodes.educational-institutions.faculties.users.edit', [$node, $educationalInstitution, $faculty, $user]) }}">
                                                    <svg class="inline p-0 m-0 h-5 w-6 mb-2 hover:cursor-pointer" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            ...
                                            @can('destroy_educational_institution_user')
                                                <x-jet-dropdown-link class="modal-open inline-block" onclick="modal('{{ route('nodes.educational-institutions.faculties.users.destroy', [$node, $educationalInstitution, $faculty, $user]) }}')">
                                                    <svg class="inline p-0 m-0 h-5 w-6 mb-2 hover:cursor-pointer" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
