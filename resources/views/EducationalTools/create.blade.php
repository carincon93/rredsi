    ...
    <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
        <div class="md:grid md:grid-cols-3 md:gap-4">
            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="POST" action="{{ route('educational-tools.store') }}">
                    @csrf
                    ...
                    <div>
                        ...
                        <x-jet-input id="qty" class="block mt-1 w-full" type="number" min="" max="" name="qty" value="{{ old('qty') }}" required />
                        ...
                        <div class="mt-2">
                            ...
                            <select id="educational_environment_id" name="educational_environment_id" class="block mt-1 p-4 w-full" value="{{ old('educational_environment_id') }}" required >
                                <option value=''>Seleccione un ambiente </option>
                                @forelse ($educationalEnvironments as $educationalEnvironment)
                                    <option selected="{{ $educationalEnvironment->name ==  old('educational_environment_id')  ? 'selected' : '' }}" value={{$educationalEnvironment->id}}>  {{$educationalEnvironment->name}} </option>
                                @empty
